...
import { Star, Check, Truck, Shield, Users, Zap, ChevronDown, MessageCircle, User, Phone, MapPin } from "lucide-react";
          ...
          <div className="space-y-3">
            {reviews.map((review) => (
              <div key={review.id} className="border rounded-md p-4" data-testid={`review-${review.id}`}>
                ...
                <p className="text-sm  text-start text-muted-foreground">{review.comment}</p>
              </div>
            ))}
          </div>
