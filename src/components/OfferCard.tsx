...
export default function OfferCard({ offer, isSelected = false, onSelect }: OfferCardProps) {
  return (
    <button
      ...
      className={`w-full rounded-md border p-4 text-right transition-all ${isSelected ? 'border-primary bg-accent' : 'hover-elevate'
        }`}
      data-testid={`button-offer-${offer.id}`}
    >
      <div className="mb-2 flex items-start justify-between gap-2">
        <div className="flex-1">
          <h3 className="mb-1 text-sm font-medium" data-testid={`text-offer-title-${offer.id}`}>
            {offer.titleAr}
          </h3>
        ...
        <div
          className={`flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full border-2 ${isSelected ? 'border-primary' : 'border-input'
            }`}
